         ...
         loop
               ...
               when track_ptr.used_by = 0 =>
                  accept allowServiceTrain( train_id : in Positive) do
                     ...
                     if pass_service_train_ptr /= null then
                        ...
                        log.putLine(ustr.To_String(type_str)&"["&Natural'Image(track_ptr.id)&"] received accept request from service train ["&Positive'Image(pass_service_train_ptr.id)&"]. Blocking track for other trains.",model_ptr);
                     else
                        Ada.Text_IO.Put_Line(ustr.To_String(type_str)&"["&Natural'Image(track_ptr.id)&"] received null pointer for service train ID["&Positive'Image(train_id)&"]");
                     end if;
               ...
               accept acceptServiceTrain( train_id : in Positive) do
                  if pass_service_train_ptr /= null and then pass_service_train_ptr.id = train_id then
                    ...
                    hist.train_id := train_id;
                     ...
                     log.putLine(ustr.To_String(type_str)&"["&Natural'Image(track_ptr.id)&"] blocked by passing service train: ["&Positive'Image(pass_service_train_ptr.id)&"]",model_ptr);
                  else
                     if track_ptr.used_by = 0 or track_ptr.used_by = train_id then
                        log.putLine(ustr.To_String(type_str)&"["&Natural'Image(track_ptr.id)&"] received accept signal from invalid serivce train ID["&Positive'Image(train_id)&"] no service train expected. Blocking the track anyway.",model_ptr);
                        pass_service_train_ptr := model.getTrain(train_id,model_ptr);
                        track_ptr.used_by := train_id;
                        train_ptr := pass_service_train_ptr;
                        ...
                        log.putLine(ustr.To_String(type_str)&"["&Natural'Image(track_ptr.id)&"] received accept signal from invalid serivce train ID["&Positive'Image(train_id)&"] no service train expected. Currently used by other train.",model_ptr);
                     end if;
                  end if;
            ...
            or
               accept freeFromServiceTrain ( train_id : in Positive ) do
                  if pass_service_train_ptr /= null and then pass_service_train_ptr.id = train_id then
                     log.putLine(ustr.To_String(type_str)&"["&Natural'Image(track_ptr.id)&"] unblocked from service train["&Positive'Image(pass_service_train_ptr.id)&"].",model_ptr);
                     track_ptr.used_by := 0;
                     train_ptr := null;
                     pass_service_train_ptr := null;
                  else
                     if pass_service_train_ptr = null then
                        Ada.Text_IO.Put_Line(ustr.To_String(type_str)&"["&Natural'Image(track_ptr.id)&"] receive free signal from invalid serivce train ID["&Positive'Image(train_id)&"] no service train accepted.");
                     else
                        Ada.Text_IO.Put_Line(ustr.To_String(type_str)&"["&Natural'Image(track_ptr.id)&"] receive free signal from invalid serivce train ID["&Positive'Image(train_id)&"] accepted: ["&Positive'Image(pass_service_train_ptr.id)&"]");
                     end if;
                  end if;
            ...
            or
               ...
               accept repair ( train_id : in Positive)  do
                  if help_service_train_ptr /= null and then help_service_train_ptr.id = train_id then
                     log.putLine(ustr.To_String(type_str)&"["&Natural'Image(track_ptr.id)&"] was just repaired. Ready to accept incoming trains anew.",model_ptr);
                     track_ptr.out_of_order := false;
                  else
                     if help_service_train_ptr /= null then
                        log.putLine(ustr.To_String(type_str)&"["&Natural'Image(track_ptr.id)&"] has no information about service train but received repair signal from service train["&Positive'Image(train_id)&"]. Accepting the repair and moving along with schedule.",model_ptr);
                        track_ptr.out_of_order := false;
                     else
                        log.putLine(ustr.To_String(type_str)&"["&Natural'Image(track_ptr.id)&"] received repair signal from illegal service train ["&Positive'Image(train_id)&"]. Accepting the repair and moving along with schedule.",model_ptr);
                        track_ptr.out_of_order := false;
                     end if;
                  end if;
               end repair;
            ...
            if track_ptr.t_type /= Track_Type_Service and then track_ptr.used_by = 0 and then track_ptr.out_of_order = false then
               ...
               if track_ptr.reliability < ran then
                  log.putLine(ustr.To_String(type_str)&"["&Natural'Image(track_ptr.id)&"] broke at time " & log.toString(log.getRelativeTime(Ada.Real_Time.Clock,model_ptr)),model_ptr);
