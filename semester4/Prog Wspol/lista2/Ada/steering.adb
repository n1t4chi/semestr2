         ...
         loop
            if steering_ptr.out_of_order = true and help = false then
               ...
               if help_service_train_ptr /= null then
                  ...
                  Ada.Text_IO.Put_Line("Steering["&Positive'Image(steering_ptr.id)&"] received null pointer for service train" );
               end if;
            end if;
            ...
            or
               ...
               accept allowServiceTrain( train_id : in Positive) do
                     ...
                     if pass_service_train_ptr /= null then
                      ...
                      --  log.putLine("Steering["&Natural'Image(steering_ptr.id)&"] received accept request from service train ["&Positive'Image(pass_service_train_ptr.id)&"]. Blocking steering for other trains.",model_ptr);
                     else
                        Ada.Text_IO.Put_Line("Steering["&Natural'Image(steering_ptr.id)&"] received null pointer for service train ID["&Positive'Image(train_id)&"]");
                     end if;
            ...
            or
               accept acceptServiceTrain( train_id : in Positive) do
                  if pass_service_train_ptr /= null and then pass_service_train_ptr.id = train_id then
                     log.putLine("Steering["&Natural'Image(steering_ptr.id)&"] blocked by passing service train: ["&Positive'Image(pass_service_train_ptr.id)&"]",model_ptr);
                     ...
                     if pass_service_train_ptr = null  then
                        if steering_ptr.used_by = 0 or steering_ptr.used_by = train_id then
                           log.putLine("Steering["&Natural'Image(steering_ptr.id)&"] received accept signal from invalid serivce train ID["&Positive'Image(train_id)&"] no service train expeted. Blocking the track anyway.",model_ptr);
                           pass_service_train_ptr := model.getTrain(train_id,model_ptr);
                           steering_ptr.used_by := train_id;
                           train_ptr := pass_service_train_ptr;
                           ...
                           log.putLine("Steering["&Natural'Image(steering_ptr.id)&"] received accept signal from invalid serivce train ID["&Positive'Image(train_id)&"] no service train expeted at is currently used by other train.",model_ptr);

                        end if;
                     else
                        log.putLine("Steering["&Natural'Image(steering_ptr.id)&"] received accept signal from invalid serivce train ID["&Positive'Image(train_id)&"] expected: ["&Positive'Image(pass_service_train_ptr.id)&"]",model_ptr);
                     end if;
                  end if;
            ...
            or
               ...
               accept freeFromServiceTrain ( train_id : in Positive ) do
                  if pass_service_train_ptr /= null and then pass_service_train_ptr.id = train_id then
                     log.putLine("Steering["&Natural'Image(steering_ptr.id)&"] unblocked from service train["&Positive'Image(pass_service_train_ptr.id)&"]",model_ptr);
                     steering_ptr.used_by := 0;
                     train_ptr := null;
                     pass_service_train_ptr := null;
                  else
                     if pass_service_train_ptr = null then
                        Ada.Text_IO.Put_Line("Steering["&Natural'Image(steering_ptr.id)&"] receive free signal from invalid serivce train ID["&Positive'Image(train_id)&"] no service train accepted.");
                     else
                        Ada.Text_IO.Put_Line("Steering["&Natural'Image(steering_ptr.id)&"] receive free signal from invalid serivce train ID["&Positive'Image(train_id)&"] accepted: ["&Positive'Image(pass_service_train_ptr.id)&"]");
                     end if;
                  end if;
            ...
            or
               ...
               accept repair ( train_id : in Positive)  do
                  if help_service_train_ptr /= null and then help_service_train_ptr.id = train_id then
                     log.putLine("Steering["&Natural'Image(steering_ptr.id)&"] was just repaired. Ready to accept incoming trains anew.",model_ptr);
                     steering_ptr.out_of_order := false;
                  else
                     if help_service_train_ptr /= null then
                        log.putLine("Steering["&Natural'Image(steering_ptr.id)&"] has no information about service train but received repair signal from service train["&Positive'Image(train_id)&"]. Accepting the repair and moving along with schedule.",model_ptr);
                        steering_ptr.out_of_order := false;
                     else
                        log.putLine("Steering["&Natural'Image(steering_ptr.id)&"] received repair signal from illegal service train ["&Positive'Image(train_id)&"]. Accepting the repair and moving along with schedule.",model_ptr);
                        steering_ptr.out_of_order := false;
                     end if;
                  end if;
            ...
            if steering_ptr.used_by = 0 and steering_ptr.out_of_order = false then
               ...
               if steering_ptr.reliability < ran then
                  log.putLine("Steering["&Natural'Image(steering_ptr.id)&"] broke at time " & log.toString(log.getRelativeTime(Ada.Real_Time.Clock,model_ptr)) ,model_ptr );
