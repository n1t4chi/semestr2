            ...
            or
               accept notifyAboutWorkerArrival (work_id : Positive) do
                  ...
                  if work_ptr /= null then
                     ...
                     Ada.Text_IO.Put_Line("#3# station[" & Positive'Image(stat_ptr.id) & "] received null pointer for worker[" & Positive'Image(work_id)&"]");
                  end if;
            ...
            if created_task then
               if HashSet.Equivalent_Sets(Left => stat_ptr.ready_workers , Right => stat_ptr.chosen_workers) then
                  ...
                  begin
                     ...
                     log.putLine("#3# station[" & Positive'Image(stat_ptr.id) & "] got notifications from all chosen workers. Notifying workers thath they can start working for next " & dur & " hours.",model_ptr);
                  end;
                  created_task := false;
                  active_task := true;
               ...
               ran := Random(G);
               if ran < 0.05 then
                  log.putLine("#3# station[" & Positive'Image(stat_ptr.id) & "] needs to have new task performed. Notifying workers",model_ptr);
                  declare
                  ...
                  begin
                     ...
                     if worker_count > 0 then
                        log.putLine("#3# station[" & Positive'Image(stat_ptr.id) & "] choose " & Natural'Image(worker_count) & " workers for task.",model_ptr);
                        created_task := true;
                     else
                        log.putLine("#3# station[" & Positive'Image(stat_ptr.id) & "] could not choose any workers for this task. Abandoning task.",model_ptr);
