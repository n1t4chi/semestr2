...
public class SortLogRadix {
  ...
  /*  public HashMap<Integer,HashMap<Integer,Comparable[]>> getSortHistory() {
     ...
     */
    public SortLogRadix(int N,PrintStream out,boolean history) {
        this.out = out;
    ...
    private static Comparable[][] deepClone(Comparable[][] tab){
        Comparable[][] rtrn = new Comparable[tab.length][tab[0].length];
     ...
     * @param always
     */
    public static void addToHistory(Comparable[][] tab/*,int step,int branch*/, SortLogRadix log,boolean always){
        if(log!=null){
     ...
     * @param log log
     */
    public static void incrementSwap(SortLogRadix log){
        if(log!=null)
     ...
     * @param log log
     */
    public static void incrementCompare(SortLogRadix log){
        if(log!=null)
     ...
     */
    public static void printlnTab(Comparable[][] tab,SortLogRadix log,boolean always){
        if(log!=null){
            if(log.out!=null)
            if(always || log.N<=20){
                boolean first_el = true;
                for(Comparable[] t : tab){
                    log.out.print(((first_el)?"{":",{")); 
                    boolean first = true; 
                    for(Comparable c : t){
     ...
     * @param always if logging is activated then whether the string will always be printed or only if sorted array size is no more than 10
     */
    public static void printlnSummary(Comparable[][] tab,SortLogRadix log,boolean always){
        if(log!=null){
            if(log.out!=null)
            if(always || log.N<=20){
     ...
     * @param always if logging is activated then whether the string will always be printed or only if sorted array size is no more than 10
     */
    public static void println(String text,SortLogRadix log,boolean always){
        if(log!=null){
            if(log.out!=null)
            if(always || log.N<=20)
    ...
    private static class tab {
        final Comparable[] tab;

        public tab(Comparable[] tab) {
            this.tab = tab;
        }
        ...
        public boolean equals(Object obj) {
            if(obj instanceof Comparable[]){
                return Arrays.equals(tab, (Comparable[]) obj);
            }else if(obj instanceof tab){
                return Arrays.equals(tab, ((tab)obj).tab);
            }
            return false;
        }
        ...
        public int hashCode() {
            int hash = 3;
            hash = 53 * hash + Arrays.deepHashCode(this.tab);
            return hash;
        }
    
    }
    public static BufferedImage historyToImage(SortLogRadix log){
        if(log!=null){
          /*  LinkedHashMap<Integer, HashMap<Integer, Comparable[]>> h = log.history.entrySet().stream().sorted((e1,e2) -> e1.getKey().compareTo(e2.getKey()))
           ...
           /*     HashMap<Integer, Comparable[]> first = h.get(h.size());
                ...
                HashMap<tab,Color> hm = new HashMap<>();
                double cr = 0;
                double cg = 0;
                double cb = 0;
                int stage = 0;
                ...
                for(int i=1;i<tab.length;i++){
                    if(!Arrays.equals(tab[i], tab[i-1]))
                        diff++;
                }
                ...
                for(int i=0;i<tab.length;i++){
                    if(!hm.containsKey(new tab(tab[i]))){
                        hm.put(new tab(tab[i]),new Color((int)cr,(int)cg,(int)cb));
                       // System.out.println(Arrays.toString(tab[i])+" color RGB("+new Color((int)cr,(int)cg,(int)cb)+")");
                        switch(stage){
                            case 0:
                                if(cb+step>=255){
                                    cg += (cb+step-255);
            ...
            /*    for(Entry<Integer, HashMap<Integer, Comparable[]>> ent : h.entrySet()){
                   ...
                   /* for(Entry<Integer, Comparable[]> entry : step_tab.entrySet()){
                    ...
                    for(Comparable[] c : t ){
                       // System.out.println("("+i+","+j+")"+Arrays.toString(c)+", col:"+hm.get(new tab(c)));
                        g.setColor(hm.get(new tab(c)));
